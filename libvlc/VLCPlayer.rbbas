...
Inherits libvlc.VLCInstance
	...
	#tag Method, Flags = &h0
		Function EventManager() As libvlc.EventReceiver
		  If mEventManager = Nil Then
		    ...
		    If e <> Nil Then 
		      mEventManager = New EventManager(e, Me)
		      AddHandler mEventManager.VLCEvent, WeakAddressOf VLCEventHandler
		    End If
		  End If
	...
	#tag Method, Flags = &h21
		Private Sub VLCEventHandler(Sender As EventManager, EventType As libvlc.EventType, Referent As Ptr, DataStruct As Ptr)
		  #pragma Unused Sender
		  Break
		End Sub
	#tag EndMethod
